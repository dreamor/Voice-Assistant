...
def set_correction_enabled(enabled: bool) -> None:
    """设置是否启用 ASR 纠错"""
...
def _needs_correction(text: str) -> bool:
    ...
    # 只有在包含明确的"电脑操作指令+可能音译词"模式时才纠错
    # 例如"打开 欧喷" "运行 派松" "安装 多克"
    tech_action_patterns = [
        r'(打开|运行|执行|启动|安装|卸载).{0,4}[\u4e00-\u9fff]{2,8}$',
        r'(帮我去?|请帮我?|帮我?打开|帮我?运行).{0,4}[\u4e00-\u9fff]{2,8}$',
    ]
